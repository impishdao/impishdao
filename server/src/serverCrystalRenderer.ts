...
class Rect {
  ...
  constructor(gen: Generator<number>, depth: number, generation: number) {
    ...
    const widths = this.get_minmax_width_at_depth(depth);
    this.bottomWidth = rb(gen, widths[0].min, widths[0].max);
    this.topWidth = rb(gen, widths[1].min, widths[1].max);

    const height = this.get_minmax_height_at_depth(depth);
...
class Finger {
  ...
  render(size: number, canvas: Canvas) {
...
export function crystal_image(seed: string, sym: number, generation: number, size: number): Buffer {
  const f = new Finger(seed, sym, generation);

  const canvas = createCanvas(700, 700);
  f.render(size, canvas);
