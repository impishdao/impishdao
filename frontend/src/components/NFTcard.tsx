...
export default function NFTCard({selectedAddress, nftPrice, buyNFTFromDAO, tokenId}: NFTCardProps) {
    ...
    return (
        <Card style={{width: '320px', padding: '10px', borderRadius: '5px'}} key={tokenId.toString()}>
            <a style={{color: 'white'}} target="_blank"  rel='noreferrer' href={`https://randomwalknft.com/detail/${paddedTokenId}`}>
            ...
            {selectedAddress && 
                <Button variant="primary" onClick={() => buyNFTFromDAO(tokenId)}>Buy Now</Button>
            }
            </Card.Body>
        </Card>
    );
